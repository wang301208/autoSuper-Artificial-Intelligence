...
4. **设置项目**
    接下来我们需要安装所需依赖。我们提供了一个工具帮助你完成仓库中的所有任务。在终端输入 `./scripts/run` 即可访问。

    首个需要使用的命令是 `./scripts/run setup`，它会引导你完成系统设置。
    最初你会收到安装 flutter、chrome 以及设置 GitHub 访问令牌的指示，如下图所示：

    > 注意：对于高级用户，GitHub 访问令牌仅在运行 `./scripts/run arena enter` 命令时需要，以便系统能自动创建 PR
...
运行 `./scripts/run setup` 时，如果遇到 `No such file or directory` 或 `FileNotFoundError` 等错误，可能是因为 Windows 风格的行结尾（CRLF）与 Unix/Linux 风格的行结尾（LF）不兼容。
...
执行上述命令后，运行 `./scripts/run setup` 应该就能成功。
...
完成设置后，下一步是创建代理模板。
执行命令 `./scripts/run agent create YOUR_AGENT_NAME`，其中 `YOUR_AGENT_NAME` 替换为你选择的名称。
...
进入 Arena 是可选步骤，适用于希望参与代理排行榜的用户。如果你决定参与，可运行 `./scripts/run arena enter YOUR_AGENT_NAME` 进入 Arena。此步骤对代理的开发或测试并非必需。
...
使用 `./scripts/run agent start YOUR_AGENT_NAME` 可以启动代理。
...
如果遇到问题并想确保代理已停止，可以使用 `./scripts/run agent stop`，该命令会终止占用 8000 端口的进程，即该代理。
...
agpt % ./scripts/run benchmark categories
...
agpt % ./scripts/run benchmark tests
...
```bash
./scripts/run benchmark start YOUR_AGENT_NAME
